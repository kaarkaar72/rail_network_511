...
import stomp
import socket
from extraction.utils.trust import *
import sys
...
TRAIN_MOVEMENTS_TOPIC = os.getenv("TRAIN_MOVEMENTS_TOPIC", "train_movements_cleaned")
...
class RailMovementListener(stomp.ConnectionListener):
    ...
    def on_message(self,frame):
        ...
        try:
            ...
            if "TRAIN_MVT_" in headers.get("destination", ""):
                print_trust_frame(parsed_body) 
                for record in parsed_body:
                    self.producer.send("rail_movement_raw", record)
                self.producer.flush()
